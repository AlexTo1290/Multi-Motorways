...
import { roadTiles, roadTilesJunctions, roadTilesJunctionsDict } from "../recoil/atom/roadAtoms";
...
const RoadPlacement= () => {
    ...
    // const addToRoadDict = useRecoilCallback(({set}) => (id, item) => {
    //     set(roadTilesJunctionsDict(id), item);
    // })

    // useEffect(() => {
    //     if (roadTilesJunctionsArr.length > numberOfRoads.current) {
    //         numberOfRoads.current = roadTilesJunctionsArr.length;
            
    //         let newId = currentId.current;
    //         currentId.current += 1;

    //         addToRoadDict(newId, roadTilesJunctionsArr[roadTilesJunctionsArr.length-1]);

    //         roads.current.push(<RoadPieceHandler key={newId} id={newId} />)
    //     }
